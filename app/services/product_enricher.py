...
Integrates:
- Supplier-specific scraping (CLF, Essential, Suma, Infinity)
- Nutrition parsing
...
async def scrape_product_data(
    ...
    # 1. Try brand website first (highest trust)
    if brand:
    ...
    # 2. Try big 4 suppliers
    for supplier_key, config in SUPPLIER_CONFIGS.items():
        if config.get("requires_login"):
            # Skip suppliers requiring login for now
            # TODO: Implement authenticated scraping
            continue
    ...
    # 3. Try fallback search if no results
    if not results or not any(r.get("ingredients") for r in results):
        query = ean if ean else product_name
        for source_name, url_pattern in FALLBACK_SEARCH_URLS[:2]:  # Limit to first 2
            try:
